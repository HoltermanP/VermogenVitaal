...
function PricingPageWithAuth() {
  const clerkAvailable = isClerkAvailable()
  const clerkData = useUser()

  // Als Clerk niet beschikbaar is, behandel als niet ingelogd
  if (!clerkAvailable) {
    return <PricingPage user={null} isLoaded={true} />
  }

  return <PricingPage user={clerkData.user || null} isLoaded={clerkData.isLoaded} />
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function PricingPage({ user: propUser, isLoaded: propIsLoaded }: { user: any, isLoaded: boolean }) {
  const user = propUser
