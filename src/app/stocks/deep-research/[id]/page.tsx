...
function DeepResearchDetailPageWithAuth() {
  const clerkAvailable = isClerkAvailable()
  const clerkData = useUser()

  // Als Clerk niet beschikbaar is, behandel als niet ingelogd
  if (!clerkAvailable) {
    return <DeepResearchDetailPage user={null} isLoaded={true} />
  }

  return <DeepResearchDetailPage user={clerkData.user || null} isLoaded={clerkData.isLoaded} />
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function DeepResearchDetailPage({ user: propUser, isLoaded: propIsLoaded }: { user: any, isLoaded: boolean }) {
  const user = propUser
...
export default function DeepResearchDetailPageWrapper() {
  return (
    <ClerkErrorBoundary>
      <DeepResearchDetailPageWithAuth />
    </ClerkErrorBoundary>
  )
}
