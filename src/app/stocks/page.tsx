...
// Wrapper component die useUser alleen aanroept als Clerk beschikbaar is
// Dit voorkomt errors wanneer Clerk niet is geconfigureerd
function StocksPageContentWithAuth() {
  const clerkAvailable = isClerkAvailable()
  const clerkData = useUser()

  // Als Clerk niet beschikbaar is, behandel als niet ingelogd
  if (!clerkAvailable) {
    return <StocksPageContent user={null} isLoaded={true} />
  }

  return <StocksPageContent user={clerkData.user || null} isLoaded={clerkData.isLoaded} />
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function StocksPageContent({ user: propUser, isLoaded: propIsLoaded }: { user: any, isLoaded: boolean }) {
  // Gebruik props of fallback naar null
  const user = propUser
