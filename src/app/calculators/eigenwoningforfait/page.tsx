...
import { calculateOwnHomeForfait, type OwnHomeForfaitResult } from "@/lib/calculators/own-home-forfait"

export default function OwnHomeForfaitCalculatorPage() {
  ...
  const [results, setResults] = useState<OwnHomeForfaitResult | null>(null)

  const handleCalculate = () => {
    const result = calculateOwnHomeForfait(formData)
    setResults(result)
  }
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-12">
            <h1 className="text-4xl font-bold text-foreground mb-3">
              <span className="text-gradient-financial">Eigenwoningforfait Calculator</span>
            </h1>
            <p className="text-lg text-muted-foreground">
              Bereken eigenwoningforfait en netto voordeel hypotheekrenteaftrek
            </p>
          </div>

          <div className="mb-6">
            <NewsTicker pagePath="/calculators/eigenwoningforfait" />
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <Card className="bg-card/80 backdrop-blur-sm border-border shadow-xl">
              ...
              <CardContent className="space-y-6">
                <div>
                  <Label htmlFor="wozValue">WOZ-waarde</Label>
                  <Input
                    id="wozValue"
                    type="number"
                    value={formData.wozValue}
                    onChange={(e) => setFormData({ ...formData, wozValue: parseFloat(e.target.value) || 0 })}
                    className="mt-1"
                  />
                </div>
                ...
                {results ? (
                  <div className="space-y-6">
                    <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                      <h3 className="font-semibold mb-3">Eigenwoningforfait</h3>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">WOZ-waarde:</span>
                          <span className="font-medium">€{Math.round(results.wozValue).toLocaleString('nl-NL')}</span>
                        </div>
                        <div className="flex justify-between border-t pt-2">
                          <span className="font-semibold">Eigenwoningforfait (0.35%):</span>
                          <span className="font-semibold">€{Math.round(results.ownHomeForfait).toLocaleString('nl-NL')}</span>
                        </div>
                      </div>
                    </div>

                    <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                      <h3 className="font-semibold mb-3">Hypotheekrenteaftrek</h3>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                    ...
                    <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                      <h3 className="font-semibold mb-3">Netto Resultaat</h3>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Eigenwoningforfait Belasting:</span>
                          <span className="font-medium">€{Math.round(results.ownHomeForfait * 0.37).toLocaleString('nl-NL')}</span>
                        </div>
                        <div className="flex justify-between border-t pt-2">
                          <span className="font-semibold">Netto Voordeel:</span>
                          <span className={`font-semibold ${results.netBenefit > 0 ? 'text-green-600' : 'text-red-600'}`}>
                            €{Math.round(results.netBenefit).toLocaleString('nl-NL')}
                          </span>
                        </div>
                        <div className="flex justify-between">
                  ...
                  <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                    <p className="text-muted-foreground text-center">Klik op &quot;Bereken Forfait&quot; om resultaten te zien</p>
                  </div>
                )}
