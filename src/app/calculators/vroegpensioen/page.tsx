...
import { calculateEarlyRetirement, type EarlyRetirementResult } from "@/lib/calculators/early-retirement"
...
export default function EarlyRetirementPage() {
  ...
  const [results, setResults] = useState<EarlyRetirementResult | null>(null)

  const handleCalculate = () => {
    const result = calculateEarlyRetirement({
      ...formData,
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-12">
            ...
            <p className="text-lg text-muted-foreground">
              Bereken kosten en mogelijkheden van vervroegd pensioen
            </p>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <Card className="bg-card/80 backdrop-blur-sm border-border shadow-xl">
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  <Label htmlFor="desiredRetirementAge">Gewenste Pensioenleeftijd</Label>
                  <Input
                    id="desiredRetirementAge"
                    type="number"
                    value={formData.desiredRetirementAge}
                    onChange={(e) => setFormData({ ...formData, desiredRetirementAge: parseInt(e.target.value) || 0 })}
                    className="mt-1"
                  />
                </div>

                <div>
                  <Label htmlFor="standardRetirementAge">Standaard Pensioenleeftijd</Label>
                  <Input
                    id="standardRetirementAge"
                    type="number"
                    value={formData.standardRetirementAge}
                    onChange={(e) => setFormData({ ...formData, standardRetirementAge: parseInt(e.target.value) || 0 })}
                    className="mt-1"
                  />
                </div>
                ...
                <div>
                  <Label htmlFor="annualSavings">Jaarlijks Sparen (€)</Label>
                  <Input
                    id="annualSavings"
                    type="number"
                ...
                {results ? (
                  <div className="space-y-6">
                    <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                      <h3 className="font-semibold mb-3">Vroegpensioen Analyse</h3>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Jaren Vroeger:</span>
                          <span className="font-medium">{results.yearsEarly} jaar</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Mogelijke Pensioenleeftijd:</span>
                          <span className="font-medium">{results.possibleRetirementAge} jaar</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Benodigd Vermogen:</span>
                        ...
                        {results.shortfall > 0 && (
                          <div className="flex justify-between border-t pt-2">
                            <span className="font-semibold text-red-600">Tekort:</span>
                            <span className="font-semibold text-red-600">€{Math.round(results.shortfall).toLocaleString('nl-NL')}</span>
                          </div>
                        )}
                        {results.shortfall === 0 && (
                          <div className="flex justify-between border-t pt-2">
                            <span className="font-semibold text-green-600">Op Koers!</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {results.shortfall > 0 && (
                      <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                        <h3 className="font-semibold mb-3">Extra Sparen</h3>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between border-t pt-2">
                            <span className="font-semibold">Extra Maandelijks:</span>
                            <span className="font-semibold text-gradient-financial">€{Math.round(results.additionalMonthlySavings).toLocaleString('nl-NL')}</span>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <div className="p-4 bg-accent/10 border border-primary/20 rounded-xl">
                    <p className="text-muted-foreground text-center">Klik op &quot;Bereken Vroegpensioen&quot; om resultaten te zien</p>
                  </div>
                )}
