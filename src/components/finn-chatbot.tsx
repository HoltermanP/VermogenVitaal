  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // Check of het een authenticatie error is
        if (response.status === 401) {
          setNeedsAuth(true)
          const errorMessage: Message = {
            role: "assistant",
            content: errorData.message || "Er is een authenticatie probleem opgetreden.",
            timestamp: new Date().toISOString()
          }
          setMessages(prev => [...prev, errorMessage])
          toast.error("Authenticatie fout", {
            description: errorData.message || "Probeer opnieuw in te loggen."
          })
          return
        }
      ...
      setMessages(prev => {
        ...
        return [...newMessages, assistantErrorMessage]
      })
